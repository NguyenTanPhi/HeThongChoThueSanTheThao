...
<head>
    ...
    <style>
        ...
        .card-goi:hover {
            ...
            box-shadow: 0 35px 70px rgba(253, 126, 20, 0.6);
        }
...
<body>
    ...
    <script>
        ...
        `,
                ...
                confirmButtonText: '<i class="fas fa-credit-card"></i> Thanh toán VNPay',
                cancelButtonText: 'Hủy',
                confirmButtonColor: '#fd7e14'
            }).then((result) => {
                if (result.isConfirmed) {
                    window.currentGoiId = id;
                    // Lưu gói dịch vụ id vào localStorage để dùng ở vnpay_return
                    localStorage.setItem('goi_dich_vu_id', id);
                    ...
                    fetch('http://127.0.0.1:8000/api/owner/thanh-toan', {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.payment_url) {
                                Swal.close();
                                // Chuyển sang link VNPay để thanh toán
